...
/**
 * @file
 * Theme override and preprocess functions for thrifthawk.
 */

/**
 * Implements hook_theme().
 */
function thrifthawk_theme() {
  $items = array();
 ...
 * - toolbar.css
 */
function thrifthawk_css_alter(&$css) {
  $exclude = array(
 ...
 */
function thrifthawk_html_head_alter(&$head_elements) {
  // Borrowed from Boron
 ...
 */
function thrifthawk_preprocess_html(&$vars) {
  ...
  $vars['classes_array'][] = drupal_html_class('layout-' . thrifthawk_active_layout());
}

/**
 * Preprocesses variables for page template.
 */
function thrifthawk_preprocess_page(&$vars) {
  ...
  $vars['title_tabs_content_wrapper_classes'] = implode(' ', _thrifthawk_region_classes('title_tabs_content'));
}

/**
 * Process page variables as late as possible.
 */
function thrifthawk_process_page(&$vars) {
 ...
 */
function thrifthawk_preprocess_maintenance_page(&$vars) {
 ...
 */
function thrifthawk_process_maintenance_page(&$vars) {
 ...
 */
function _thrifthawk_region_classes($region) {
  $layouts = _thrifthawk_layouts();
  $layout = thrifthawk_active_layout();
 ...
 */
function thrifthawk_active_layout() {
  static $layout;
...
/**
 * Returns all thrifthawk layouts registered by modules and themes.
 */
function _thrifthawk_layouts() {
  static $layouts;
  if (!isset($layouts)) {
    ...
    drupal_alter('thrifthawk_layouts', $layouts);
  }
  return $layouts;
}
 ...
 */
function thrifthawk_preprocess_region(&$vars) {
  $vars['classes_array'] = array_merge($vars['classes_array'], _thrifthawk_region_classes($vars['region']));
}

/**
 * Changes the search form to use the "search" input element of HTML5.
 */
function thrifthawk_preprocess_search_block_form(&$vars) {
  // Borrowed from Boron
 ...
 */
function thrifthawk_pager($vars) {
  $tags = $vars['tags'];
 ...
 */
function thrifthawk_pager_list($vars) {
  $tags = $vars['tags'];
  $element = $vars['element'];
  $parameters = $vars['parameters'];
  $quantity = $vars['quantity'];
 ...
 */
function thrifthawk_pager_link($vars) {
  $text = $vars['text'];
